	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw' => __('This block will show all <a href="edit.php?post_type=oum-location">Locations</a> on a map. By default users will be able to propose new locations by clicking a + Button on the map.', 'open-user-map') . '<br><br>' . __('Please configure the map styles and features in <a class="link-oum-settings" href="options-general.php?page=open_user_map">Settings > Open User Map</a>.', 'open-user-map'),
		...
		$this->add_control(
			...
			[
				'label' => '',
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => __('Feel free to customize initial map position (Latitude, Longitude & Zoom).<br><br>This will override the general configuration from the <a href="options-general.php?page=open_user_map">settings</a>.', 'open-user-map'),
				'content_classes' => 'oum-elementor-howto-description',
			]
		);
	...
	protected function render() {
		...
		<?php if(Plugin::is_elementor_backend()): ?>
			...
			<div class="hint" style="height: <?php echo $settings['oum_map_height']; ?>px">
				...
				<?php if($settings['latitude'] != '' || $settings['longitude'] != '' || $settings['zoom'] != '' || $settings['types'] != '' || $settings['ids'] != ''): ?>
		...
		<?php else: ?>
			...
			echo do_shortcode('[open-user-map '. $lat . ' ' . $long . ' ' . $zoom . ' '. $types . ' '. $ids . ' '. $size .' '. $size_mobile .' '. $height .' '. $height_mobile .']'); 
			?>
