...
class Settings extends BaseController
{
    ...
    public function add_plugin_settings()
    {
        ...
        register_setting('open-user-map-settings-group', 'oum_enable_searchaddress');
    ...
    public function migrate_deprecated_settings()
    {
        $options = array(
            ...
            'oum_disable_searchaddress' => 'oum_enable_searchaddress',
        );
    ...
    {
        if((!get_option( 'oum_wizard_usecase_done' ) || !get_option( 'oum_wizard_finish_done' )) && !get_option('oum_enable_add_location')) {
    ...
    public function csv_import()
    {
        ...
        if(isset($_POST['action']) && $_POST['action'] == 'oum_csv_import') {
            ...
            if($error->has_errors()) {
                ...
                $csv_file = get_home_path() . wp_make_link_relative($_POST['url']);
                $delimiter = $this->detectDelimiter($csv_file);

                // parse csv file to array
                $file_to_read = fopen($csv_file, 'r');
                while (!feof($file_to_read) ) {
                    $rows[] = fgetcsv($file_to_read, 1000, $delimiter);
                }            
                fclose($file_to_read);
